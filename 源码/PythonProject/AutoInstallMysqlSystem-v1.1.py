...
# @Time    : 2020/1/16
# @Author  : lee
# @FileName: AutoInstall.py
# @Desc    : 远程服务器MySQL自动安装系统
#
...
class LinkServer():
    ...
    def mkdir_shell_file(self, shellName, context):
        with open(shellName, "w", encoding='utf-8') as file:  # ”w"代表着每次运行都覆盖内容
    ...
    def check_mysql(self):
        ...
        # std_in, std_out, std_err = ssh_client.exec_command('mysql --help | grep Distrib')
        # std_in, std_out, std_err = ssh_client.exec_command('mysql -V')
    ...
    def install_mysql(self):
        ...
        # 清除本地shell文件
        os.remove(self.uninstall_local_path)
        os.remove(self.install_local_path)
    ...
    def start_install(self):
        ...
        if self.serverPass == 0:
            ...
            installorNot = tkinter.messagebox.askyesno('提示', '安装MySQL将耗费约' + str(
                len(self.ipcode) * 2) + '分钟, 请保持电脑网络连接正常。\n (具体时间因服务器处理速度或网络带宽有所差异）'
                                        '\n是否继续安装？')
            if installorNot:
                for num in range(len(self.ipcode)):
                    b = LinkServer(self.ipcode[num], self.sPwd[num], self.mysqlPwd[num], self.version[num])
